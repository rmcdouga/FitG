...
public class BaseGameCounterRenderer {
	...
	private enum ImperialMilitaryUnitRenderer implements Renderer {
		...
		public String render() {
			return ColorsAndStyles.STYLE_IMPERIAL.render(symbol);
		}
	...
	private static String renderCounters_System(Collection<Counter> counters) {
		...
		if (containsImperialCounters(counters)) {
			sb.append(ColorsAndStyles.STYLE_IMPERIAL.render(CIRCLE_I)); // Circle I
		}
		if (containsRebelCounters(counters)) {
			sb.append(ColorsAndStyles.STYLE_REBEL.render(CIRCLE_R)); // Circle R
		}
		return sb.toString();
	}
	...
	private static String renderCounter_Planetary(Counter counter) {
		...
		return RENDERERS_MAP.getOrDefault(counter.id(), ()->"*" + counter.id()).render();

...
public class BaseGameEnvironRenderer {
	...
	public enum BaseGameEnvironTypeRenderer {
		...
		private BaseGameEnvironTypeRenderer(BaseGameEnvironType environType, AttributedStyle style, String symbol) {
	...
	static AttributedString renderEnviron(ZoomLevel zoomLevel, Environ environ) {
		return renderEnviron(zoomLevel, BaseGameEnviron.requireBgEnviron(environ));
	}

	static AttributedString renderEnviron(ZoomLevel zoomLevel, BaseGameEnviron environ) {
		...
		return new AttributedString("%s %s".formatted(renderer.symbol, environ.getSize()), renderer.style);
	}
}